...
export async function runBatch({ total = 1000, chunkSize = 50, onProgress, onSample }) {
  const ratios = { block: 0.07, challenge: 0.13 }
  let banned = 0
  let challenged = 0
  let approved = 0
  let valueBlocked = 0
  ...
  const countries = ['KZ', 'KZ', 'KZ', 'RU', 'TR', 'NG', 'VN', 'US']

  for (let c = 0; c < chunks; c += 1) {
    await delay(16)
    for (let j = 0; j < chunkSize; j += 1) {
      const i = c * chunkSize + j
      if (i >= total) break
      const roll = hash(`fh-batch-${i}`) / 100
      const amount = 500 + (hash(`amt-${i}`) % 900) * 1200 + (hash(`big-${i}`) % 3) * 800000
      let bucket
      if (roll < ratios.block) {
        bucket = STATUS.BLOCK
        banned += 1
        valueBlocked += amount
      } else if (roll < ratios.block + ratios.challenge) {
        bucket = STATUS.CHALLENGE
        challenged += 1
      } else {
        bucket = STATUS.APPROVE
        approved += 1
      }
      if (i < 8) {
        sample.push({
      ...
      onSample?.({ index: i + 1, total, amount, status: bucket, timestamp: Date.now() })
    }
    onProgress?.({ progress: ((c + 1) / chunks) * 100, index: (c + 1) * chunkSize, total })
  }
  ...
  return {
    ...
    blocked: banned,
    valueBlocked,
    fpr: ((challenged + banned) * 0.04).toFixed(2),
    saved: valueBlocked,
    sample,
    elapsedMs: chunks * 16,
  }

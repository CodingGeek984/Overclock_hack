...
import { STATUS } from '../utils/riskColors.js'
import {
  CODE_TO_COUNTRY,
  countryCodeToName,
  countryToCode,
...
export function mapBackendStats(stats) {
  ...
  return {
    ...
    precisionPct: Number(stats.precision) || (total > 0 ? ((safe / total) * 100) : 0),
    recallPct: Number(stats.recall) || 0,
    optimalThreshold: Number(stats.optimal_threshold) || 60
  }
}
...
export default function XaiCenter() {
  ...
  const [txId, setTxId] = useState(TRANSACTIONS[0].id)
  const [topN, setTopN] = useState(5)

  const tx = useMemo(() => TRANSACTIONS.find((row) => row.id === txId) ?? TRANSACTIONS[0], [txId])
  const params = useMemo(() => toParams(tx), [tx])

  const assessment = useMemo(() => buildAssessment(params, 42), [params])
        ...
        <div className="rounded-[28px] bg-white border border-zinc-200 p-6 sm:p-7 space-y-5">
          ...
          <div className="pt-1 border-t border-zinc-100">
            ...
            <div>
              ...
              <SnapshotRow label={t.country} value={`${tx.city}, ${tx.country}`} />
